...
class DebugRenderers extends InheritedWidget {
  ...
  DebugRenderers({List<ValueRenderer> renderers, @required Widget child})
      : this.renderers = <ValueRenderer>[]..addAll(renderers ?? {}),
        super(child: child);
  ...
  static Widget build(BuildContext context, dynamic value,
    ...
    var renderer = renderers.firstWhere((r) => r.support(value: value),
        orElse: () => null);
    if (renderer == null) {
  ...
  static Future<dynamic> prompt(BuildContext context, Sink sink) {
    final renderers = of(context);
    var renderer =
        renderers.firstWhere((r) => r.support(sink: sink), orElse: () => null);
    if (renderer == null) {
      renderer = renderer ?? DefaultRenderer();
    }
